            ...
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <form id="cotizacionesForm" action="{{ route('cotizaciones.update', $cotizacione->id) }}" method="post">
                    ...
                    <div class="mb-4">
                        ...
                        <input type="date" class="form-input mt-1 block w-full rounded-md border-gray-300 @error('vigencia') border-red-500 @enderror" id="vigencia" name="vigencia" value="{{ $cotizacione->vigencia }}" min="{{ $cotizacione->created_at->format('Y-m-d') }}">
                        @error('vigencia')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
                    <div class="mb-4">
                        ...
                        <input type="number" class="form-input mt-1 block w-full rounded-md border-gray-300 @error('cantidad') border-red-500 @enderror" id="cantidad" name="cantidad" value="{{ $cotizacione->cantidad }}">
                        @error('cantidad')
                        <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                    </div>
