    ...
    <div class="md:w-4/4 mx-auto">
        ...
        <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
            <div class="p-6 bg-white border-b border-gray-200">
                ...
                <a href="{{ route('vendedores.create') }}" class="bg-green-500 hover:bg-green-700 font-bold py-2 px-4 rounded inline-flex items-center mb-4">
                ...
                <div class="overflow-x-auto">
                    ...
                    <table id="Table" class="min-w-full divide-y divide-gray-200">
                        ...
                        <tbody class="bg-white divide-y divide-gray-200">
                            @forelse ($vendedores as $vendedore)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $vendedore->id }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $vendedore->nombre }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $vendedore->telefono }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $vendedore->correo }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <form id="deleteForm" action="{{ route('vendedores.destroy', $vendedore->id) }}" method="post">
                                        @csrf
                                        @method('DELETE')
                                        <a href="{{ route('vendedores.show', $vendedore->id) }}" class="bg-yellow-500 hover:bg-yellow-700 font-bold py-1 px-2 rounded"><i class="bi bi-eye"></i>Mostrar</a>
                                        <a href="{{ route('vendedores.edit', $vendedore->id) }}" class="bg-blue-500 hover:bg-blue-700 font-bold py-1 px-2 rounded"><i class="bi bi-pencil-square"></i>Editar</a>
                                        <button type="submit" class="bg-red-500 hover:bg-red-700 font-bold py-1 px-2 rounded" onclick="return confirm('¿Estás seguro de eliminar este vendedor?');">
